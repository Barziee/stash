...
export function QuickAdd() {
  ...
  return (
    <div className="bg-card rounded-2xl p-4 flex flex-col gap-3">
      <p className="text-[9px] tracking-[0.2em] text-muted-foreground/50 uppercase">הוספה מהירה</p>
          ...
          return (
            ...
            >
              <div
                ...
                style={{
                  backgroundColor: active ? c.color + '33' : 'var(--secondary)',
      ...
      <div className="flex items-center gap-2">
        <div className="flex-1 flex items-center gap-1.5 bg-secondary rounded-xl px-3 py-2.5">
          ...
          <input
            ...
            className="flex-1 bg-transparent text-foreground text-base font-semibold outline-none placeholder:text-muted-foreground/20 min-w-0"
          />
        </div>
        <button
          onClick={handleAdd}
      ...
      {selectedCategoryId === null && (
        <p className="text-[10px] text-muted-foreground/30 text-center">בחר קטגוריה ולאחר מכן הזן סכום</p>
      )}
    </div>
  );
}
