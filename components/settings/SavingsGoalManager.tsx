      ...
      {goals?.map(g => {
        ...
        return (
          <div key={g.id} className="bg-card border border-border rounded-xl p-3">
            <div className="flex justify-between items-start mb-2">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">₪{g.currentAmount.toFixed(0)} / ₪{g.targetAmount.toFixed(0)}</p>
                </div>
              </div>
              <div className="flex items-center gap-1">
                <Button
                  variant="ghost" size="icon" className="h-6 w-6 text-[var(--income)]"
            ...
            <p className="text-[10px] text-muted-foreground mt-1 text-end">{pct.toFixed(0)}%</p>
          </div>
        );
      })}

      <div className="flex flex-col gap-2 border border-border rounded-xl p-3 mt-1">
